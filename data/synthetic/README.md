...
| `synthetic_real_sym.csv` | 500 | Paraphrase-only rewrites again, but generated with the same instruction and the same per-article length target as `synthetic_fake_sym.csv`. Pairs with it to form C2′/C3′; `synthetic_real.csv` is untouched | `generate_synthetic_real.py --symmetric` |
